...
%Using stress tensors with 2-norm = 1 bounds the nRSS values by [0 0.5]
...
num_orientations = 1000; %number of orientations to sample
...
stress = sig_025x1y; %applied stress tensor
...
for ii = 1:num_orientations
    ...
    for jj = 1:length(rot_steps) %loop through rotations
        ...
        for ll = 1:30
            ...
            tauR(:,ll) = abs(scalar22(squeeze(R(:,:,ll)),stress)); %resolved shear stress
        end
